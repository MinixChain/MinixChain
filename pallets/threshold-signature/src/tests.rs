...
fn exec_script_no_dump_signature() {
    new_test_ext().execute_with(|| {
        ...
        // assert_noop!(Pallet::<Test>::pass_script(Origin::signed(who), addr, signature_ab.clone(), ab.clone(), control_block.clone(), 666667u32.to_be_bytes().to_vec(), script_hash.clone()), Error::<Test>::InvalidSignature);
        // assert_eq!(pallet_balances::Pallet::<Test>::free_balance(who), 20);
        // assert_eq!(pallet_balances::Pallet::<Test>::free_balance(who), 20);
    });
}
